...
int proc_entry(const char *file_path_src, const struct stat *info,
...
int proc_entry(const char *file_path_src, const struct stat *info,
	...
	for (int i = 0; i < num_exclude; i++)
		if (0 == strcmp(exclude[i], file_path_src)) add = 0;
	...
	if (add) {
		...
		if (typeflag == FTW_D) {
			if (file_path_src[strlen(file_path_src) - 1] != '/') {
				...
				snprintf(file_path_dst, MAX_FILE_PATH_LENGTH,
					 "%s/", file_path_src);
			}

			mz_zip_writer_add_mem(ptr_zip, file_path_dst, NULL, 0,
					      0);
		} else if (typeflag == FTW_F)
			mz_zip_writer_add_file(ptr_zip, file_path_dst,
					       file_path_src, "", 0,
					       MZ_BEST_COMPRESSION);
	}
